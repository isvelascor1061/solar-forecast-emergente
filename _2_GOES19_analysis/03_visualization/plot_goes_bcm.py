...
# BCM: 2-category discrete colormap
BCM_COLORS = ["#2ECC71", "#EA580C"]
BCM_LABELS = ["Clear", "Cloudy"]
BCM_TICKS  = [0, 1]
BCM_CMAP   = mcolors.ListedColormap(BCM_COLORS)
...
def try_variable_names(ds):
    """
    Try candidate variable names for BCM in order.
    Returns (name, DataArray) or raises KeyError if none found.
    """
    # Prefer BCM (binary 0/1) over the 4-category ACM variable
    candidates = ["BCM", "Mask", "ACM"]
    ...
    raise KeyError("No suitable BCM variable found in dataset.")
...
def download_bcm(utc_dt: datetime):
    ...
    Returns
    -------
    (bcm_vals, lons, lats) cropped to the study domain, or None on failure.
    """
    try:
        ...
        with fs.open(s3_path, "rb") as f:
            ...
            bcm_crop = bcm_da.values.astype(float)[np.ix_(row_idx, col_idx)]
            ...
            bcm_crop[~mask_crop] = np.nan

            return bcm_crop, lons_sub, lats_sub
