...
Standalone PyTorch model definitions for symmetrische (oder kausale)
LSTM‑Vorhersagen auf den Sequenz‑Dateien aus `prepare_sequences.py`.

Importierbar in einem späteren `train_lstm.py` – dort kannst du
Datasets, Optimizer, Training‑Loop etc. bauen.
...
class LSTMRegressorsigmoid(nn.Module):
...
if __name__ == "__main__":
    ...
    model1 = LSTMRegressorsigmoid(n_feat=F, seq_len=L, hidden=64)
    print("One‑step params:", model1.n_parameters)
    yhat = model1(x)
    print("Output shape:", yhat.shape)   # (B,)

    # Many‑to‑Many (Beispiel m=24)
    model2 = LSTMRegressorsigmoid(n_feat=F, seq_len=L, hidden=(64,32), multi_step=24)
